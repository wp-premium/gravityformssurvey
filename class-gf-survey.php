...
class GFSurvey extends GFAddOn {
	...
	public function validation( $validation_result ) {
		...
		foreach ( $form['fields'] as &$field ) {
			...
			if ( 'likert' == $input_type && rgar( $field, 'gsurveyLikertEnableMultipleRows' ) && rgar( $field, 'isRequired' ) ) {
				...
				foreach ( $rows as $row ) {
					if ( $i % 10 == 0 ) {
						$i ++; // skip numbers ending in 0. so that 5.1 doesn't conflict with 5.10
					}
	...
	public function merge_tag_filter( $value, $merge_tag, $options, $field, $raw_value ) {
		...
		if ( $input_type == 'likert' && rgar( $field, 'gsurveyLikertEnableMultipleRows' ) ) {
			...
			if ( is_array( $value ) ) {
				...
				uksort( $value, array( $this, 'compare_likert_values' ) );
	...
	public function get_survey_field_content( $content, $field, $value, $lead_id, $form_id, $lead = null ) {
		...
		switch ( $sub_type ) {
			case 'likert' :
				...
				for ( $i = 1; $i <= $num_rows; $i ++ ) {
					...
					foreach ( $field['choices'] as $choice ) {
						...
						$id = $form_id . '_' . $field['id'] . '_' . $row_id . '_' . $choice_id;
			...
			case 'rating' :
				...
				foreach ( $field['choices'] as $choice ) {
					$id = $form_id . '_' . $field['id'] . '_' . $choice_id ++;
	...
	public function localize_scripts() {
		...
		$rating_strings = array(
			'firstChoice'  => __( 'Terrible', 'gravityformssurvey' ),
			'secondChoice' => __( 'Not so great', 'gravityformssurvey' ),
			'thirdChoice'  => __( 'Neutral', 'gravityformssurvey' ),
			'fourthChoice' => __( 'Pretty good', 'gravityformssurvey' ),
			'fifthChoice'  => __( 'Excellent', 'gravityformssurvey' )
		);
	...
	public function contacts_tab( $contact_id ) {
		...
		if ( false === empty( $contact_id ) ) :
			...
			if ( empty( $entries ) ) :
				_e( 'This contact has not submitted any survey entries yet.', 'gravityformssurvey' );
			else :
				?>
	...
	public function add_survey_field( $field_groups ) {
		...
		foreach ( $field_groups as &$group ) {
			if ( $group['name'] == 'advanced_fields' ) {
				$group['fields'][] = array( 'class' => 'button', 'value' => __( 'Survey', 'gravityformssurvey' ), 'onclick' => "StartAddField('survey');", 'data-type' => 'survey' );
			}
		}
